...
export async function hasDashboardPermission(userId, featureKey) {
    try {
        ...
        const { data: userPermission } = await admin
            .from('dashboard_user_permissions')
            .select('is_enabled')
            .eq('user_id', userId)
        ...
        if (userPermission) {
            return userPermission.is_enabled === true
        }

        // 2. Fallback to role-based permission
        const { data: rolePermission, error: permError } = await admin
            .from('dashboard_role_permissions')
            .select('is_enabled')
            .eq('organization_id', profile.organization_id)
            .eq('role', profile.role)
        ...
        return rolePermission?.is_enabled === true
    } catch (error) {
