...
/**
 * GET /api/inventory/units/[id]/deals
 * Returns all deals for a unit, ordered by status priority, with creator/updater profiles.
 */
export const GET = withPermission('view_deals', async (request, context) => {
    try {
        ...
        const deals = await DealService.getDealsByUnit(unitId, profile.organization_id)
        const enriched = await enrichDealsWithProfiles(deals)
        return NextResponse.json({ deals: enriched })
    } catch (error) {
        console.error('Error fetching unit deals:', error)
